...
class M_index extends CI_Model {
	...
	function laporan_harian2($tanggal, $number, $offset)
	{
		$this->db->where('tanggal', $tanggal);
	...
	function row_periode2($mulai, $sampai)
	{
		$this->db->where('tanggal >=', $mulai);
		$this->db->where('tanggal <=', $sampai);
        $query = $this->db->get('data1');
	...
	function laporan_periode2($mulai, $sampai, $number, $offset)
	{
		$this->db->where('tanggal >=', $mulai);
		$this->db->where('tanggal <=', $sampai);
        $this->db->order_by('jenis ASC');
    ...
    function clean()
    {
        $query = $this->db->truncate('data','data1');
        return $query;
    }
}
